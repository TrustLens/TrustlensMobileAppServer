...
public class Controller {
	...
	private static Question[] questions =  	{	new Question("data_collection", 		"Data collection",					2, ""),
			    								...
			    								new Question("purpose", 				"Purpose of collected data",		3, prefixes + "SELECT DISTINCT ?objective ?rationale ?objectiveDescription WHERE { <http://trustlens.org/test_dataset1#HomeMonitoringHighLevelPlan> a ep-plan:Plan; ep-plan:includesObjective ?objective. ?objective a ep-plan:Objective; rdfs:comment ?objectiveDescription; ep-plan:hasRationale ?rationale. } " ),
			    								new Question("sharing", 				"Data Sharing",						3, prefixes + "SELECT  ?a ?b ?c  WHERE {     ?a ?b ?c . }  LIMIT 1"),
			    								
			    								new Question("access", 					"Data Access",						1, ""),
			    								new Question("contact", 				"Contact Information",				1, ""),
    ...
    private static IotSystem[] iotsystems = {
    											new LocatedIotSystem ("id1","Meston Air quality sensor", "Sensor description", "Meston Buidling, Aberdeen", 0, 0, "AB24 3EU", default_iot_system_uri, ""),
    											new LocatedIotSystem ("id2","SDRL Motion sensor", "Sensor description", "Sir Duncan Rice Library, Aberdeen", 0, 0, "AB24 3AA", default_iot_system_uri, ""),
    											new QrcodeLocatedIotSystem ("id3","Tillydrone Ballard", "Sensor description", "Tillydrone, Aberdeen", 1, 1, "AB24 2UD", "AAA", "", ""),
	...
	public  ResponseEntity<String>  question(@RequestBody questionPostParameters parameters) {
		...
		String response = queryexecutor.executeQuestion(parameters.getQuestion_id(), getIotSystemFromId(parameters.getSystem_id()));
    ...
    public  ResponseEntity<String>  old_question(@RequestBody questionPostParameters parameters) {
    	...
    	String queryString = retrieveSparqlQueryString(parameters.getQuestion_id())
				.replace("\"", "")
				.replace("\n", " ");
    	...
    	response = queryexecutor.executeQuestion(parameters.getQuestion_id(), getIotSystemFromId(parameters.getSystem_id()));
    ...
    public ResponseEntity<IotSystem[]>  locatesystems(@RequestBody locatesystemsPostParameters parameters) {
    	...
    	if (parameters.getMode() == 1 )  {
    		...
    		locatediotsys = retrieveLocatedSystem(parameters.getLatitude(), parameters.getLongitude(), parameters.getRadius());
    	}
    ...
    private Set<String>  getPostcodesUriOrdnancesurvey(double lat, double lon, int radius) {
    	...
    	String sparqlQueryString = 	"Select ?postcode WHERE{?postcode <http://www.w3.org/2003/01/geo/wgs84_pos#lat> ?lat . ?postcode <http://www.w3.org/2003/01/geo/wgs84_pos#long> ?long . ?postcode a <http://data.ordnancesurvey.co.uk/ontology/postcode/PostcodeUnit>.?postcode <http://data.ordnancesurvey.co.uk/ontology/postcode/district> <http://data.ordnancesurvey.co.uk/id/7000000000030421>" + 
    								" FILTER (?lat > " + String.valueOf(minlat) + " && ?lat < " + String.valueOf(maxlat) + " && ?long < " + String.valueOf(minlon) + " && ?long > " + String.valueOf(maxlon) + " ) } ";
    	
    	System.out.println(sparqlQueryString);
    	// sparqlQueryString = "Select ?postcode WHERE{?postcode <http://www.w3.org/2003/01/geo/wgs84_pos#lat> ?lat . ?postcode <http://www.w3.org/2003/01/geo/wgs84_pos#long> ?long . ?postcode a <http://data.ordnancesurvey.co.uk/ontology/postcode/PostcodeUnit>.?postcode <http://data.ordnancesurvey.co.uk/ontology/postcode/district> <http://data.ordnancesurvey.co.uk/id/7000000000030421>FILTER (?lat > 57.15314438363484 && ?lat < 57.17113081636515  && ?long < -2.09208 && ?long > -2.09209) }";
    	System.out.println(sparqlQueryString);
    ...
    private  Set<IotSystem> retrieveIotsytemByPostcode (String postcode) {
    	...
    	String queryString = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> " + 
    			...
    			"?platform prov:atLocation " + "<"+postcode+">" +  " . " +
    			"OPTIONAL { " + 
        ...
        while(results.hasNext()) {
            ...
            if (system!= null) {
            	...
            	if (highLevelsystem!=null)
            		set.add(new LocatedIotSystem(system.toString(), label.toString(), comment.toString(), "", 0, 0, pcode, uri, highLevelsystem.toString(), highLevelsystemLabel.toString(), highLevelsystemComment.toString(), "" ) );
            	else
            		set.add(new LocatedIotSystem(system.toString(), label.toString(), comment.toString(), "", 0, 0, pcode, uri, "" ) );
...
// "SELECT DISTINCT ?step  WHERE { <http://trustlens.org/test_dataset1#HomeMonitoringHighLevelPlan> a ep-plan:Plan; ep-plan:includesStep ?step. ?step a ep-plan:Step.  } "